...
where pr.Distance = 600 group by pr.Bus_Tpye; 
...
/* 10)Display the passenger name and percentage of distance travalled bt that passenger
 from the total distance travalled by all the passengers without using user variables.*/
 
 select Passenger_name,Disntance*100/(select sum(distance) from PASSENGER) as percentage from PASSENGER;
